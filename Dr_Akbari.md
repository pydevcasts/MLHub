...
### 📥 داده‌های اصلی از جدول (داده‌ها از اسکرین‌شات‌ها استخراج شده‌اند):
...
### میانگین و انحراف معیار هر ویژگی:

$$
\begin{align*}
\mu_{F1} &= \frac{2+3+4+3+4+5}{6} = 3.5 \\
\sigma_{F1} &= \sqrt{\frac{(2-3.5)^2 + \dots + (5-3.5)^2}{6}} = \sqrt{\frac{7.5}{6}} ≈ 1.12 \\
\mu_{F2} &= 4.0 \quad \sigma_{F2} = 1.29 \\
\mu_{F3} &= 4.0 \quad \sigma_{F3} = 1.41 \\
\end{align*}
$$

(مقادیر انحراف معیار با دقت بالا محاسبه شدند.)

---

## ✅ داده نرمال‌شده:

| Sample | F1 (Z) | F2 (Z) | F3 (Z) | Class |
| ------ | ------ | ------ | ------ | ----- |
| x1     | -1.34  | 0.00   | 0.71   | 1     |
| x2     | -0.45  | -0.78  | 0.00   | 1     |
| x3     | 0.45   | -1.56  | 1.41   | 1     |
...
## ✅ محاسبه واریانس در کلاس‌ها (روی داده نرمال‌شده):

### F1:

* کلاس 1: Var = var(\[-1.34, -0.45, 0.45]) = 0.85
* کلاس 2: Var = var(\[-0.45, 0.45, 1.34]) = 0.85
* وزن = |0.85 - 0.85| = **0.00**

### F2:

* کلاس 1: Var = var(\[0.00, -0.78, -1.56]) ≈ 0.85
* کلاس 2: Var = var(\[1.56, 0.78, 0.00]) ≈ 0.85
* وزن = **0.00**

### F3:

* کلاس 1: Var = var(\[0.71, 0.00, 1.41]) = 0.50
* کلاس 2: Var = var(\[-1.41, -0.71, 0.00]) = 0.50
* وزن = **0.00**

😐 همه اختلاف‌ها صفر شدند چون بعد از نرمال‌سازی، پراکندگی‌ها در کلاس‌ها دقیقاً مشابه شده‌اند (در این مجموعه‌ی داده‌ی ساده).

---

## ✅ حالا روش **شما**:

طبق حرف شما، اول **نرمال‌سازی** کنیم، بعد **واریانس کل هر ویژگی** را در تمام داده‌ها محاسبه کنیم و از آن به‌عنوان وزن ویژگی استفاده کنیم.

### واریانس کل ویژگی‌ها روی داده نرمال‌شده:
...
(دقیقاً ۱ شده چون با Z-Score نرمال شده‌اند.)

بنابراین، اگر از واریانس کلی برای تعیین وزن استفاده کنیم، همه ویژگی‌ها وزن برابر می‌گیرند.

---

## ✅ نتیجه‌گیری نهایی:

| روش                    | وزن F1 | وزن F2 | وزن F3 | نتیجه                                         |
| ---------------------- | ------ | ------ | ------ | --------------------------------------------- |
| روش من بدون نرمال‌سازی | 0.00   | ≈0.67  | ≈0.67  | منطقی، چون برخی ویژگی‌ها تفاوت پراکندگی دارند |
| روش من + نرمال‌سازی    | 0.00   | 0.00   | 0.00   | اختلاف واریانس حذف شد                         |
| روش شما (variance کل)  | 1.00   | 1.00   | 1.00   | همه برابر، چون با Z نرمال‌شده                 |
